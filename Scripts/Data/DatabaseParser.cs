        ...
        for (int i = 0; i < lines.Length; i++)
        {
            ...
            else if (actualizar)
            {
                if(metaCount < metas.Length){
                    ...
                    "                    prerrequisitos = Util.strEnum"+metas[metaCount]+"[1].Split('_'),\n"+
...
public class DataExporter
{
    ...
    public DataExporter(SQLiteConnection dbConnection)
    {
        sqliteDB = dbConnection;
    }

    public void ExportData(string tabla)
    {
        ...
        {
            case "Enum":
                dbParser.DatosEnumToFile(sqliteDB, tabla, outputPath);
            break;
            case "Meta":
                dbParser.DatosMetaToFile(sqliteDB, tabla, outputPath);
            break;
        }
    }
}
