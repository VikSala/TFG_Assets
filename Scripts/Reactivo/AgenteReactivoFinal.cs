...
public class AgenteReactivoFinal : AgentePushdownAutomata
{
    ...
    protected override void Start()
    {
        ...
        //if(controladorEstados.CambiarEstado(EstadoAgenteExistencia.Hambre))
                //TomarDecisiones(Hambre);
    }

    void TomarDecisiones(GameObject target)
    {
        estadoActual = controladorEstados.ObtenerEstadoActual();
        agenteDeliberativo.ObjetivoTemporal = target;
        agenteDeliberativo.instancias[target.name.Split("_")[0]].Add(target.name);//agenteDeliberativo.memoria.Add(target.name);
        ...
        switch (estadoActual)
        {
            ...
            case EstadoAgenteExistencia.Hambre:
                agenteDeliberativo.newMessage(Util.StrEnum(EstadoAgenteExistencia.Hambre));//target = Hambre.transform.position;
                //navMeshAgent.SetDestination(target); Util.Print("El agente tiene hambre.", isDebug);
                break;
            case EstadoAgenteExistencia.Sed:
                agenteDeliberativo.newMessage(Util.StrEnum(EstadoAgenteExistencia.Sed));
                break;
            case EstadoAgenteExistencia.Somnolencia:
                agenteDeliberativo.newMessage(Util.StrEnum(EstadoAgenteExistencia.Somnolencia));
                break;
            case EstadoAgenteExistencia.Amenaza:
                agenteDeliberativo.newMessage(Util.StrEnum(EstadoAgenteExistencia.Amenaza));
                break;
            case EstadoAgenteExistencia.Peligro:
                agenteDeliberativo.newMessage(Util.StrEnum(EstadoAgenteExistencia.Peligro));
                break;
        }
    }
    ...
    protected override void InputSed()
    {
        if(controladorEstados.CambiarEstado(EstadoAgenteExistencia.Sed)){
            agenteDeliberativo.newMessage(Util.StrEnum(EstadoAgenteExistencia.Sed));
            agenteDeliberativo.NuevoEstado(Util.StrEnum(EstadoAgenteExistencia.Sed), true);
        }
    }
    protected override void InputHambre()
    {
        if(controladorEstados.CambiarEstado(EstadoAgenteExistencia.Hambre)){
            agenteDeliberativo.newMessage(Util.StrEnum(EstadoAgenteExistencia.Hambre));
            agenteDeliberativo.NuevoEstado(Util.StrEnum(EstadoAgenteExistencia.Hambre), true);
        }
    }
    protected override void InputSomnolencia()
    {
        if(controladorEstados.CambiarEstado(EstadoAgenteExistencia.Somnolencia)){
            agenteDeliberativo.newMessage(Util.StrEnum(EstadoAgenteExistencia.Somnolencia));
            agenteDeliberativo.NuevoEstado(Util.StrEnum(EstadoAgenteExistencia.Somnolencia), true);
        }
    }
                        ...
                        if (hit.collider.CompareTag("Player")) {
                            ...
                            switch (hit.collider.gameObject.name)
                            {
                                ...
                                case string a when a.Equals(Util.StrEnum(EstadoAgenteExistencia.Peligro)):
                                    isAlerta = true; TomarDecisiones(hit.collider.gameObject);
                                    break;
                            }
                            if(endInteraction){
